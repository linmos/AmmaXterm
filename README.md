...
Bring your own API key and get a hand from a side-panel assistant: turn plain
English into commands, explain a command or a baffling error message, or analyze
terminal output. Pick your provider — Claude, any OpenAI-compatible endpoint,
Google Gemini, or a fully local Ollama model. It's private by design: nothing is
sent until you ask, only the text you select or type ever leaves your machine,
obvious secrets are scrubbed before sending, and any suggested command is simply
inserted into your prompt for you to review — never run on its own. It stays off
...
**🤖 AI 助手(選用)**
自備 API 金鑰,就能在側邊面板獲得協助:把白話需求變成指令、解釋某個指令或看不懂
的錯誤訊息,或分析終端機輸出。供應商任你挑 —— Claude、任何 OpenAI 相容端點、
Google Gemini,或完全在本機執行的 Ollama 模型。它以隱私為先:未經詢問不會送出任何
內容,只有你選取或輸入的文字會離開電腦,送出前會先遮蔽明顯的機密,而 AI 建議的指令
只會「填入」你的輸入列供你檢視 —— 絕不自動執行。預設關閉,需自行開啟。
